...
{
  tmf8829Driver * driver = (tmf8829Driver *)dptr;
  tofResults.layout_byte = data[ TMF8829_PRE_HEADER_SIZE + 1 ];
...
void handleReceivedResultData ( void * dptr, uint8_t * data, uint16_t size )
{
  ...
  // Compute pixel size from layout byte captured in the frame header
  uint8_t layout    = tofResults.layout_byte;
  uint8_t numPeaks  = layout & TMF8829_CFG_RESULT_FORMAT_NR_PEAKS_MASK;
  uint8_t useSignal = ( layout & TMF8829_CFG_RESULT_FORMAT_SIGNAL_STRENGTH_MASK ) ? 1 : 0;
  uint8_t useNoise  = ( layout & TMF8829_CFG_RESULT_FORMAT_NOISE_STRENGTH_MASK  ) ? 1 : 0;
  uint8_t useXtalk  = ( layout & TMF8829_CFG_RESULT_FORMAT_XTALK_MASK           ) ? 1 : 0;
  uint8_t pixelSize = ( numPeaks * ( 3 + 2 * useSignal ) ) + ( 2 * useNoise ) + ( 2 * useXtalk );
  if ( pixelSize == 0 ) pixelSize = 3; // safety fallback

  // Byte offset to the first peak's distance within each pixel
  uint8_t distOffset = ( 2 * useNoise ) + ( 2 * useXtalk );

  uint16_t zones = size / pixelSize;
  if ( zones > TOF_MAX_ZONES ) zones = TOF_MAX_ZONES;
  for ( uint16_t i = 0; i < zones; i++ )
  {
    uint8_t * peak = data + i * pixelSize + distOffset;
    tofResults.distance_mm[i] = (uint16_t)peak[0] | ( (uint16_t)peak[1] << 8 );
    tofResults.confidence[i]  = peak[2];
  }
  tofResults.num_zones = zones;
  tofResults.ready = true;
